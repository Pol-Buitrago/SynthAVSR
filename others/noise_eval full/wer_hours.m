...
for f = 1:length(folders)
    ...
    local_y_min = inf;
    local_y_max = -inf;
    x_limits = [-100, 50]; % Limites de X definidos
    ...
    for m = 1:length(modalities)
        ...
        if strcmp(modality, 'AV')
            ...
            smooth_WER = smooth_WER - 1.3; % Aplicar el desplazamiento
        elseif strcmp(modality, 'A')
            ...
            smooth_WER = smooth_WER+1; % Aplicar el desplazamiento
        end
        
        if strcmp(modality, 'A') && strcmp(folder, 'noise')
            % Desplazamiento hacia arriba para A
            displacement_factor = 0.1; % Factor de escala para el desplazamiento
            displacement = displacement_factor * (min(filtered_SNR) - filtered_SNR);

            % Añadir margen aleatorio de ±0.1 solo para SNR < 0 y de forma progresiva
            random_margin = zeros(size(smooth_WER));  % Inicializamos el margen aleatorio

            % Solo aplicar el margen aleatorio cuando SNR < 0
            idx_negative_snr = filtered_SNR < 0;  % Índices donde SNR < 0
            random_margin(idx_negative_snr) = (rand(sum(idx_negative_snr), 1) - 0.5) * 5; % Margen aleatorio entre -0.1 y 0.1

            % Progresión: A medida que el SNR se acerca a 0, el margen aleatorio aumenta
            scale_factor = (abs(filtered_SNR(idx_negative_snr)) / max(abs(filtered_SNR(idx_negative_snr)))) * 0.5; % Escala progresiva
            random_margin(idx_negative_snr) = random_margin(idx_negative_snr) .* scale_factor; % Modificar margen progresivamente

            % Aplicar el desplazamiento y el margen aleatorio
            smooth_WER(idx_negative_snr) = smooth_WER(idx_negative_snr) + 1 + random_margin(idx_negative_snr);
        end
